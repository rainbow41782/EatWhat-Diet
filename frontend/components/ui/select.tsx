...
const triggerVariants = cva(
  [
    ...
    "text-[13px] h-10 px-3 min-w-[160px]",
  ...
  ],
  {
    variants: {
      variant: {
        bordered: "border border-white/10 bg-white/5 text-white hover:bg-white/10",
          ...
          <span className="flex items-center gap-2 min-w-0 flex-1">
            {Icon && (
              <Icon
                size={16}
                strokeWidth={1.5}
                className="shrink-0 text-muted-foreground transition-[color,stroke-width] duration-100 group-hover:text-foreground group-hover:stroke-[2]"
              />
            )}
            <span className="min-w-0 flex-1 text-left truncate">{label ?? <span className="text-white/50">{placeholder}</span>}</span>
          </span>
          <svg
            width={16}
            height={16}
            ...
            className="shrink-0 text-white/50 transition-colors duration-100 group-hover:text-white"
          >
            <path d="M6 9l6 6 6-6" />
          </svg>
        </button>
    ...
    (e: React.KeyboardEvent) => {
      ...
      if (["ArrowDown", "ArrowUp", "ArrowRight", "ArrowLeft"].includes(e.key)) {
        ...
        if (currentIdx === -1) {
          ...
          const next = ["ArrowDown", "ArrowRight"].includes(e.key) ? (currentIdx + 1) % items.length : (currentIdx - 1 + items.length) % items.length;
          items[next]?.focus();
        }
      } else if (e.key === "Home") {
        e.preventDefault();
    ...
    <SelectContentContext.Provider value={{ registerItem, activeIndex, checkedIndex }}>
      <div style={{ position: "fixed", top: triggerRect.bottom + 6, left: triggerRect.left, minWidth: triggerRect.width, zIndex: 50 }}>
        <motion.div
          ...
          className={cn(
            `relative flex flex-col gap-0.5 max-h-[300px] overflow-y-auto ${shape.container} bg-[#0d1624]/95 shadow-[0_10px_30px_rgba(0,0,0,0.28)] border border-white/10 p-1 select-none outline-none backdrop-blur-xl`,
            className
          )}
        ...
        >
          <AnimatePresence>
            {checkedRect && (
              <motion.div
                className={`absolute ${shape.bg} bg-emerald-400/20 pointer-events-none`}
                initial={false}
          ...
          <AnimatePresence>
            {activeRect && (
              <motion.div
                ...
                className={`absolute ${shape.bg} bg-white/10 pointer-events-none`}
                initial={{
                  opacity: 0,
          ...
          <AnimatePresence>
            {focusRect && (
              <motion.div
                className={`absolute ${shape.focusRing} pointer-events-none z-20 border border-primary/80`}
                initial={false}
...
const SelectItem = forwardRef<HTMLDivElement, SelectItemProps>(({ className, children, icon: Icon, value, index, disabled = false, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isActive || isChecked ? "text-white" : "text-white/70",
        disabled && "opacity-50 pointer-events-none",
        className
      )}
      {...props}
    >
      {Icon && <Icon size={16} strokeWidth={isActive || isChecked ? 2 : 1.5} className="shrink-0 transition-[color,stroke-width] duration-100" />}
...
const SelectLabel = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn("px-2 py-1.5 text-[11px] text-white/50", className)} {...props} />
));
SelectLabel.displayName = "SelectLabel";

const SelectSeparator = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} role="separator" className={cn("my-1 -mx-1 h-px bg-white/10", className)} {...props} />
));
